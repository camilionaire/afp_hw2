...
selectCell field survey st =
  -- moved continue below and deleted $, first 2 cases do nothing
  case index (cover st) (focus st) of
    Flagged -> continue st
    Uncovered -> continue st
    -- third case, uses uncoverCell fn to find new state, if gameLost
      -- or gameWon, then quits game, otherwise continues.
    Covered -> do
      let st' = st { cover = uncoverCell (focus st) survey (cover st) }
      if (gameLost field (cover (st')) || gameWon field (cover (st')))
      then halt st' 
      else continue st'

-- handles the flagging and unflagging of cells.  flagged cells
-- have different behavior in the uncover state fn in Minesweeper.hs
flagCell ::
  KnownBounds w h =>
  Field w h -> Survey w h ->
  AppState w h -> EventM Void (Next (AppState w h))
flagCell field survey st = 
  -- if the focused on cell is already uncovered, do nothing, otherwise
  -- switch off between flagged and covered.
  continue $ case index (cover st) (focus st) of
    Uncovered -> st
    Covered -> st { cover = replace (focus st) Flagged (cover st)}
    Flagged -> st { cover = replace (focus st) Covered (cover st)}
...
main :: IO ()
main = do
  -- this was me trying to put a cap on number of mines possible.
  let flags = 7
  field <- randomField @8 @10 flags
