...
-- which we write as regular old numbers, like 0 or 5 or 123. The Nat kind and
...
-- this was for testing the test functions.
-- replace _ x _ = pure x --imap (\j y -> if i == j then x else y)
...
neighborhood ::
-- takes in a grid and index with the constraint of knownbounds for w and h
-- returns an array / vector of type a, same as the grid type.
  forall w h a.
  KnownBounds w h =>
  Grid w h a -> Index w h -> [a]
neighborhood g (r,c) =
...
-- index :: Grid w h a -> Index w h -> a
-- catMaybes :: [Maybe a] -> [a]
-- uncurry :: (a -> b -> c) -> (a, b) -> c
-- liftA2 (,) :: Applicative f => f a -> f b -> f (a, b)
-- uncurry (liftA2 (,)) :: Applicative f => (f a, f b) -> f (a, b)
-- dec :: Finite n -> Maybe (Finite n)
-- inc :: KnownNat n => Finite n -> Maybe (Finite n)
-- we use fmap (uncurry (liftA2 (,))) to convert the inc / dec (maybe, maybe)
-- to a List of Maybe (Finite n, Finite n)
-- the catMaybes take away the Nothings, and the last fmap (index g) gets
-- all the value from those indices.
